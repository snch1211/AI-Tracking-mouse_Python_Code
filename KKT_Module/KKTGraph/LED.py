...
class Led(QPushButton):
    ...
    def __init__(self, parent, on_color=green, off_color=black,
                 shape=rectangle, build='release', autoOff=0):
        super().__init__()
        self.autoOff = autoOff
        self.OffTimer = QtCore.QTimer()
        self.OffTimer.timeout.connect(lambda: self.turn_off())
        self.setFont(QtGui.QFont('Yu Gothic UI', 10))
        if build == 'release':
    ...
    def set_status(self, status):
        ...
        if self._status is True:
            self._toggle_on()
            if self.autoOff != 0:
                self.OffTimer.start(self.autoOff * 1000)
        else:
            self._toggle_off()
            if self.OffTimer.isActive():
                self.OffTimer.stop()
            ...
            for s in self._shape:
                for c in self._color:
                    exec('self._{}_{} = Led(self, on_color=Led.{}, \
                          shape=Led.{}, build="debug", autoOff=2)'.format(s, c, c, s))
